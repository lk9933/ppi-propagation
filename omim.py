...
def main():
    ...
    # Filter for Deafness
    deafness_df = omim_df[omim_df['Phenotype'].str.contains('|'.join(deafness_terms), case=False, na=False)]

    # Filter for Retinitis
    retinitis_df = omim_df[omim_df['Phenotype'].str.contains('|'.join(retinitis_terms), case=False, na=False)]
    ...
    deafness_df = deafness_df.rename(columns={"Gene/Locus And Other Related Symbols": "Gene"})
    retinitis_df = retinitis_df.rename(columns={"Gene/Locus And Other Related Symbols": "Gene"})
    ...
    deafness_df = deafness_df.drop(columns=['MIM Number', 'Cyto Location'])
    retinitis_df = retinitis_df.drop(columns=['MIM Number', 'Cyto Location'])
    ...
    deafness_df['Gene'] = deafness_df['Gene'].apply(lambda x: [gene.strip() for gene in x.split(',')])
    retinitis_df['Gene'] = retinitis_df['Gene'].apply(lambda x: [gene.strip() for gene in x.split(',')])
    ...
    deafness_df = deafness_df.explode('Gene')
    retinitis_df = retinitis_df.explode('Gene')
    ...
    deafness_df = deafness_df.drop_duplicates(subset=['Gene'])
    retinitis_df = retinitis_df.drop_duplicates(subset=['Gene'])
    ...
    deafness_genes = list(deafness_df['Gene'].unique())
    retinitis_genes = list(retinitis_df['Gene'].unique())
    ...
    deafness_output_file = os.path.join(output_dir, "deafness_genes.txt")
    retinitis_output_file = os.path.join(output_dir, "retinitis_genes.txt")
    ...
    with open(diabetes_output_file, 'w') as f:
        for gene in diabetes_genes:
            f.write(f"{gene}\n")
    with open(deafness_output_file, 'w') as f:
        for gene in deafness_genes:
            f.write(f"{gene}\n")
    with open(retinitis_output_file, 'w') as f:
        for gene in retinitis_genes:
            f.write(f"{gene}\n")
    ...
    print(f"Deafness genes saved to {deafness_output_file}")
    print(f"Retinitis genes saved to {retinitis_output_file}")
